...
archAffix(){
    case "$(uname -m)" in
      ...
      's390x')
        ...
        ;;
        *)
        echo "Error: la arquitectura del sistema no es compatible."
        exit 1
        ;;
    esac
...
downloadV2Ray(){
    ...
    download_link="https://raw.githubusercontent.com/emirjorge/multi-v2ray-x3/master/update/v2ray-linux-64-v5.7.0/v2ray-linux-64.zip"
...
getVersion(){
    if [[ -n "$version" ]]; then
        ...
        if [[ $? -ne 0 ]] || [[ $new_ver == "" ]]; then
            colorEcho ${red} "No se pudo buscar el lanzamiento de la nueva versión. Por favor verifique su red o inténtelo nuevamente."
            return 3
...
stopV2ray(){
    colorEcho ${blue} "Apagando el servicio $key service."
    ...
    if [[ $? -ne 0 ]]; then
        colorEcho ${yellow} "No se pudo cerrar el servicio $key service."
        return 2
    fi
    return 0
}
...
startV2ray(){
    ...
    if [[ $? -ne 0 ]]; then
        colorEcho ${yellow} "No se pudo iniciar el servicio $key service."
        return 2
    fi
    return 0
}
...
installV2Ray(){
    ...
    chmod +x /usr/bin/$key_lower/$key_lower $chmod_param || {
        colorEcho ${red} "No se pudieron copiar los binarios y recursos de $key."
        return 1
    }
...
remove(){
    if [[ -n "${systemctl_cmd}" ]] && [[ -f "/etc/systemd/system/$key_lower.service" ]];then
        ...
        if [[ $? -ne 0 ]]; then
            ...
            colorEcho ${green} "$key removido exitosamente."
...
main(){
    ...
    if [[ $local_install -eq 1 ]]; then
        ...
        if [[ $retval == 0 ]] && [[ "$force" != "1" ]]; then
            ...
            colorEcho ${blue} "Instalando $key ${new_ver} on ${arch}"
    ...
    if [ -n "${extract_only}" ]; then
        colorEcho ${blue} "Extrayendo el archivo zip de $key a ${vsrc_root}."

        if unzip -o "${zipfile}" -d ${vsrc_root}; then
            colorEcho ${green} "$key extraido en ${vsrc_root%/}${ziproot:+/${ziproot%/}}, y saliendo..."
            return 0
        else
    ...
    if [[ ${v2ray_running} -eq 1 ]];then
        colorEcho ${blue} "Reiniciando $key service."
        startV2ray
    fi
